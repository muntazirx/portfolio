...
This is my writeup of what I actually tested: the Copy Fail 2 xfrm and ESP path. Dirty Frag is related and important context, especially because of the RxRPC bug, but I do not want to pretend I demonstrated both paths when I only ran one PoC in my VM.
...
Dirty Frag is related, but it is not the exact same thing I demonstrated. Dirty Frag chains the xfrm and ESP issue with an RxRPC issue to cover more environments. My screenshots below show the xfrm and ESP path only.
...
Copy Fail 2 and Dirty Frag sit in the same family, but they should not be treated as the exact same demo. My test was Copy Fail 2 through xfrm and ESP.
...
Copy Fail 2 does it through xfrm and ESP. Dirty Frag chains xfrm and ESP with RxRPC.
...
This part matters even though I only tested the xfrm and ESP path.
...
Dirty Frag is relevant because it points to the same kind of result through another kernel subsystem, but I am keeping that separate from my own demo.
...
2. Check whether your distribution has shipped guidance or patches for [CVE-2026-43500](https://www.cve.org/CVERecord?id=CVE-2026-43500), the related RxRPC issue I did not test.
