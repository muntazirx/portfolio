...
const GLYPH_SIZE = "h-5 w-5"; // base glyph size before optical scaling

const icons: Record<string, ReactNode> = {
  GitHub: (
    <svg viewBox="0 0 24 24" className={`${GLYPH_SIZE} transform scale-100`} fill="currentColor" aria-hidden>
  ...
  LinkedIn: (
    <svg viewBox="0 0 24 24" className={`${GLYPH_SIZE} transform scale-[0.90]`} fill="currentColor" aria-hidden>
  ...
  Email: (
    <svg viewBox="0 0 24 24" className={`${GLYPH_SIZE} transform scale-[1.05]`} fill="currentColor" aria-hidden>
      ...
      {socialLinks.map((link) => (
        <li key={link.name}>
          <a
            ...
            className={`inline-flex ${ICON_BUTTON_SIZE} items-center justify-center rounded hover:text-accent focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40`}
